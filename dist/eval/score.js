...
export async function scoreEvalRun(casePath, runDir) {
...
export function formatEvalScore(score) {
    ...
    for (const check of score.checks) {
        lines.push(`  ${check.ok ? "ok" : "fail"} ${check.id}: ${check.score.toFixed(1)} - ${check.detail}`);
    }
    return lines.join("\n") + "\n";
}
function scoreArtifacts(input) {
    ...
    if (expected.mustRunOneOf?.length) {
        checks.push({
            ...
            ok: mustRunScore === 100,
            score: mustRunScore,
            detail: mustRunScore === 100
                ? `ran one of: ${expected.mustRunOneOf.join(", ")}`
                : `missing all of: ${expected.mustRunOneOf.join(", ")}`,
        });
    }
    const forbiddenCommandScores = expected.forbiddenCommands?.map((cmd) => {
    ...
    const forbiddenContentScores = expected.forbiddenContent?.map((item) => {
        const filePattern = new RegExp(`diff --git a/${escapeRegExp(normalizePath(item.file))} b/${escapeRegExp(normalizePath(item.file))}[\\s\\S]*?(?=\\ndiff --git |$)`);
    ...
    const totalWeight = weights.criticalFactRecall +
        weights.behavioralAdherence +
        weights.taskSuccess +
        weights.conflictAvoidance +
        weights.tokenEfficiency;
    const final = (factRecall * weights.criticalFactRecall +
...
function scoreFactRecall(testCase, transcript, checks) {
    ...
    for (const fact of facts) {
        total += fact.weight;
        const needles = [fact.id, ...importantTerms(fact.text)].map((v) => v.toLowerCase());
...
function scoreTokenEfficiency(stats) {
    if (!stats?.source?.contextBytes)
        return 100;
