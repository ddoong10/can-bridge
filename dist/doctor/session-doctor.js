...
export async function diagnoseSession(locator, options = {}) {
    if (!locator)
    ...
    const formatToValidate = expectedFormat && expectedFormat !== "unknown" ? expectedFormat : detectedFormat;
    if (expectedFormat &&
    ...
    const status = findings.some((f) => f.level === "error")
        ? "fail"
 ...
 */
export async function diagnoseSessionFromContext(ctx) {
    const findings = [];
        ...
        for (let j = 0; j < m.content.length; j++) {
            ...
            if (!validBlocks.has(b.type)) {
                findings.push({
                    level: "warn",
                    code: "UNKNOWN_BLOCK",
                    message: `message ${i + 1}, block ${j + 1}: unknown type "${b.type}"`,
                });
            }
        }
    }
    ...
    const status = findings.some((f) => f.level === "error")
        ...
        : findings.some((f) => f.level === "warn")
            ? "warn"
            : "ok";
    const detectedFormat = ctx.source.tool === "claude-code"
        ? "claude-code"
...
export function formatDoctorResult(result) {
    const lines = [];
    lines.push(`Doctor ${result.detectedFormat}: ${result.status} (${result.score}/100)`);
    lines.push(`File: ${result.filePath}`);
    if (result.expectedFormat)
        lines.push(`Expected: ${result.expectedFormat}`);
    ...
    if (result.findings.length === 0) {
        ...
        for (const finding of result.findings) {
            ...
            lines.push(`- [${finding.level}] ${finding.code}${line}: ${finding.message}`);
        }
    }
    return lines.join("\n") + "\n";
}
...
async function resolveSessionPath(locator, format) {
...
function looksLikePath(locator) {
    return (locator.endsWith(".jsonl") ||
        locator.includes("/") ||
        locator.includes("\\") ||
...
function validateClaude(lines, findings) {
    const uuids = new Set();
    const parentRefs = [];
...
function validateClaudeContent(content, line, findings) {
...
function validateCodex(lines, findings) {
    ...
    for (const { line, value } of lines) {
        ...
        if (type === "session_meta") {
            ...
            responseItemCount++;
            validateCodexResponseItem(value.payload, line, findings, functionCallIds, outputRefs);
        }
...
function validateCodexSessionMeta(payload, line, findings) {
...
function validateCodexResponseItem(payload, line, findings, functionCallIds, outputRefs) {
...
function validateCodexMessageItem(payload, line, findings) {
...
function validateCodexEvent(payload, line, findings) {
