...
import { formatMessages, listInbox, listThread, readMessages, sendMessage, } from "../collab/mailbox.js";
import { diagnoseSession, formatDoctorResult, } from "../doctor/session-doctor.js";
    ...
    switch (sub) {
        ...
        case "import": {
            ...
            try {
                ...
                throw new Error(`Invalid JSON in --in: ${err instanceof Error ? err.message : String(err)}`);
            }
        ...
        case "share": {
            ...
            if (!sessionId && args.latest) {
                ...
                sessionId = latest.id;
                console.error(`Latest ${source.id} session: ${latest.id}` +
            ...
            if (written) {
                console.error(`Wrote ${written} (${pkg.messages.length} messages` +
                ...
                console.error(`  can-bridge import --to <claude-code|codex> --in ${written}`);
            }
            break;
        }
        case "pipe": {
            ...
            console.error(`Extracted ${ctx.messages.length} messages from ${ctx.source.tool}` +
...
async function runContinue(args) {
    ...
    if (args.latest) {
        console.error(`Selected latest ${from} session: ${session.id}` +
    ...
    if (args.verbose || doctor.status === "warn") {
        ...
        console.error(`Doctor ${doctor.detectedFormat}: ${doctor.status} (${doctor.score}/100)`);
    }
    ...
    switch (action) {
        case "run": {
            const result = await runEvalCase({
                ...
                condition: parseEvalCondition(typeof args.condition === "string" ? args.condition : "converted"),
                agent: parseEvalAgent(requireArg(args, "agent")),
                cwd: typeof args.cwd === "string" ? args.cwd : undefined,
                outDir: typeof args.out === "string" ? args.out : undefined,
                source: args.source === "codex" || args.source === "claude-code"
                    ? args.source
                    : undefined,
                sourceSession: typeof args["source-session"] === "string"
                    ? args["source-session"]
                    : undefined,
            ...
            if (result.scorePath)
                console.error(`Score: ${result.scorePath}`);
            break;
        }
        case "score": {
            const score = await scoreEvalRun(requireArg(args, "case"), requireArg(args, "run"));
            if (args.json) {
...
export async function pickLatestSession(source) {
    if (!source.listSessions) {
...
function formatOrigin(session) {
    const parts = [session.originator, session.sourceLabel].filter((v) => Boolean(v));
    if (session.importedFrom) {
        parts.push(`from ${session.importedFrom}` +
...
function printContinueResult(to, result) {
    console.error(`Injected to: ${result.locator}`);
    const sessionId = typeof result.details?.sessionId === "string"
    ...
    if ((to === "codex" || to === "codex-cli") && sessionId) {
        ...
        console.error(`  codex exec --skip-git-repo-check resume ${sessionId} "<your prompt>"`);
        return;
    }
    console.error(result.hint);
}
async function runMailbox(argv) {
    const [action, ...rest] = argv;
    const args = parseArgs(rest);
...
function pickTarget(id) {
    ...
    if (!factory) {
        throw new Error(`Unknown target "${id}". Available: ${Object.keys(TARGETS).join(", ")}`);
    }
    return factory();
}
function renderAsPrompt(ctx) {
    ...
    lines.push("");
    lines.push("Please continue this conversation. Pick up where the assistant left off.");
    return lines.join("\n") + "\n";
}
function printHelp() {
...
function parseEvalCondition(value) {
    if (value === "original" ||
        value === "converted" ||
        value === "no-context" ||
        value === "human-target") {
    ...
    throw new Error(`Unknown eval condition "${value}". Use original, converted, no-context, or human-target.`);
}
function parseEvalAgent(value) {
