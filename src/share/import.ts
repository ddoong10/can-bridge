...
export async function readPackage(filePath: string): Promise<CbctxPackage> {
  ...
  try {
    ...
    throw new Error(
      `Invalid .cbctx (not valid JSON): ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  if (!isCbctxPackage(parsed)) {
    throw new Error(
      `Invalid .cbctx — schema check failed (expected can-bridge.context.v1)`,
    );
  }
  return parsed;
}
...
export function packageToContext(
  pkg: CbctxPackage,
  opts: { useNative?: boolean } = {},
): NormalizedContext {
...
function restoreNativeRaw(
  pkg: CbctxPackage,
): NormalizedContext["raw"] | null {
  ...
  if (computeNativeContentHash(a.content) !== a.contentHash) return null;
...
export async function importPackage(
  ...
  if (typeof pkg.contentHash === "string" && pkg.contentHash.length > 0) {
    if (opts.skipHashVerify) {
      ...
      if (expected !== actual) {
        throw new Error(
          `Package contentHash mismatch — file may be tampered or corrupted.\n` +
            `  expected: ${expected}\n` +
            `  actual:   ${actual}\n` +
    ...
    throw new Error(
      `Package contentHash missing — integrity cannot be verified.\n` +
  ...
  if (!opts.skipDoctor) {
    try {
      ...
      if (dr.status === "fail") {
        throw new Error(
          `Doctor preflight failed (${dr.score}/100). Run with --skip-doctor to override. Findings: ${dr.findings
...
export function formatImportSummary(s: ImportSummary): string {
  const lines: string[] = [];
  lines.push(
    `Originally from ${s.source.tool}` +
      (s.source.model ? ` (${s.source.model})` : "") +
      `, ${s.messageCount} messages.`,
  );
  if (s.source.sessionId) lines.push(`  Original session: ${s.source.sessionId}`);
  if (s.source.cwd) lines.push(`  Original cwd: ${s.source.cwd}`);
  if (s.repo?.remote) {
    lines.push(
      `  Repo: ${s.repo.remote}` +
  ...
  if (s.doctor) {
    lines.push(
      `  Doctor (at share time): ${s.doctor.status} ${s.doctor.score}/100`,
    );
  }
  if (s.budget) {
    lines.push(
      `  Package budget: ${s.budget.mode}` +
  ...
  if (s.preflightStatus) {
    lines.push(
      `  Doctor (preflight on import): ${s.preflightStatus} ${s.preflightScore}/100`,
    );
  }
  if (s.hashStatus) {
    const note =
...
function resolveImportBudgetOptions(
  opts: ImportPackageOptions,
): Parameters<typeof applyContextBudget>[1] {
...
function shouldReportImportBudget(stats: ContextBudgetStats): boolean {
  return (
    stats.sinceCompact ||
