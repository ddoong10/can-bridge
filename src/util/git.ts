...
export function captureGitState(cwd: string): GitState | null {
  ...
  if (commit) state.commit = commit;
  if (status !== null) state.dirty = status.length > 0;
...
export function formatGitState(g: GitState): string {
  const parts: string[] = [];
  if (g.branch) parts.push(g.branch);
  if (g.commit) parts.push(`@ ${g.commit}`);
 ...
 *   safe match.
 */
export function gitMismatchWarning(
  source: GitState | undefined,
  current: GitState | null,
): string | null {
  if (!source) return null;
  if (!current) {
    return (
      `Could not verify the current workspace git state (source was ` +
      `${formatGitState(source)}). Re-read files before editing — historical ` +
      `tool outputs may not match disk.`
    );
  }
  const commitDiffers =
    !!source.commit && !!current.commit && source.commit !== current.commit;
  const dirtyDiffers =
    source.dirty !== undefined &&
    current.dirty !== undefined &&
    source.dirty !== current.dirty;
  if (!commitDiffers && !dirtyDiffers) return null;
  return (
    `Workspace moved since capture: source was ${formatGitState(source)}, ` +
